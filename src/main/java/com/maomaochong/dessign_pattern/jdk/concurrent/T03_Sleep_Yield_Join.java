...
public class T03_Sleep_Yield_Join {
    ...
    static void testSleep() {
        new Thread(() -> {
            for (int i = 0; i < 20; i++) {
                System.out.println(new Date() + "-A" + i);
                try {
                    Thread.sleep(1000);
    ...
    /*Yield,就是当前线程正在执行的时候停止下来进入等待队列，回到等待队列里在系统的调度算 法里头呢还是依然有可能把你刚回去的这个线程拿回来继续执行，当然，更大的可能性是把原来等待的那些拿 出一个来执行，
    ...
    static void testYield() {
        ...
        new Thread(() -> {
            for (int i = 0; i < 100; i++) {
                System.out.println(new Date() + "------------B" + i);
                if (i % 10 == 0) {
                    Thread.yield();
                }
            }
    ...
    /*join， 意思就是在自己当前线程加入你调用Join的线程()，本线程等待。等调用的线程运行 完了，自己再去执行。
